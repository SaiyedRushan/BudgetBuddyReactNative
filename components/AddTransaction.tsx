...
export default function AddTransaction({ insertTransaction }: { insertTransaction(transaction: Transaction): Promise<void> }) {
  ...
  useEffect(() => {
    getExpenseTypeFromCurrentTab(currentTab);
  }, [currentTab]);

  async function getExpenseTypeFromCurrentTab(currentTab: number) {
    setCategory(currentTab === 0 ? "Expense" : "Income");
    const type = currentTab === 0 ? "Expense" : "Income";
    const result = await db.getAllAsync<Category>(`SELECT * FROM Categories WHERE type = ?;`, [type]);
      ...
      {isAddingTransaction ? (
        <View>
          <Card>
            ...
            />
            <TextInput placeholder="Description" style={{ marginBottom: 15 }} onChangeText={setDescription} />
...
function CategoryButton({ id, title, isSelected, setTypeSelected, setCategoryId }: { id: number; title: string; isSelected: boolean; setTypeSelected: React.Dispatch<React.SetStateAction<string>>; setCategoryId: React.Dispatch<React.SetStateAction<number>> }) {
  return (
    <TouchableOpacity
      onPress={() => {
